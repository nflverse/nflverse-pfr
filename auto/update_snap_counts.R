...
scrape_snaps <- function() {
  ...
  scrape_games <- scrape_games |>
    dplyr::mutate(
      team = ifelse(location == "home", home_team, away_team),
  ...
  piggyback::pb_upload(
    file = here::here("build/snap_counts.rds"),
  ...
  new_snaps |>
    ...
    write.csv(
      here::here("build/scraped_games.csv"),
      quote = TRUE,
      row.names = FALSE
    )
  ...
  cli::cli_alert_success(
    "Finished scraping snap counts for {nrow(game_ids)} game{?s}"
  )

  return(TRUE)
}

summarise_snap_counts <- function(
  summary_season = nflreadr::most_recent_season()
) {
  cli::cli_process_start(
    "Uploading snap counts for {summary_season} to nflverse-data"
  )
